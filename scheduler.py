...
Full daily schedule with anti-ban timing gaps built in.
"""
...
async def scheduler_loop():
    morning_done = afternoon_done = organic_done = evening_done = ""
    inplay_running = False
    ...
    while True:
        ...
        # 12:00 — Organic caption post (no graphic)
        ...
        # 16:00 — Afternoon preview with videos
        if h==16 and m<5 and afternoon_done != today:
            afternoon_done = today
            logger.info("Running afternoon job with videos...")
            from jobs.run import run_morning_job
            await run_morning_job(render_videos=True, max_posts=2)

        # X reply scan — every 15 mins during peak hours
        ...
        # In-play scan — every 3 mins between 12:00 and 23:00
